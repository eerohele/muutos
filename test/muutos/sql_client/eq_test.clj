...
(defn clear-db! [& {:keys [port]}]
  (with-open [client (connect :port port)]
...
(deftest ^:integration read-error
  ...
  (let [proxy (disruptor/proxy :bind-port 10080 :connect-port 5433)]
    (with-open [pg (connect {:port 10080})]
...
(deftest ^:integration numeric
  (with-open [pg (connect-test)]
    ...
    (is (same? 9223372036854775808M
          (eq1 pg "n" ["SELECT 9223372036854775808 AS n"])))
    ...
    (is (same? 9223372036854775808M
          (eq1 pg "n" ["SELECT $1 AS n" 9223372036854775808M])))
